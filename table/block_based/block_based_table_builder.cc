...
void BlockBasedTableBuilder::partUpload(){
  const int current_part = part_number.fetch_add(1);
  auto buffer_copy = std::move(partBuffer);

  std::string local_bucket_name = bucket_name;
  std::string local_object_key = object_key;
  std::string local_upload_id = upload_id;
  auto s3client = AsyncIOScheduler::Instance().GetS3Client();

  tracker_->AddTask();

  AsyncIOScheduler::Instance().Submit([this,
                                       s3client,
                                       local_bucket_name,
                                       local_object_key,
                                       local_upload_id,
                                       current_part,
                                       buffer_copy = std::move(buffer_copy)]() mutable {

    constexpr int kMaxRetries = 10;
    IOStatus final_status = IOStatus::IOError("待上传");

    for (int attempt = 1; attempt <= kMaxRetries; ++attempt) {
      Aws::S3::Model::UploadPartRequest uploadPartRequest;
      uploadPartRequest.SetBucket(local_bucket_name);
      uploadPartRequest.SetKey(local_object_key);
      uploadPartRequest.SetUploadId(local_upload_id);
      uploadPartRequest.SetPartNumber(current_part);
      uploadPartRequest.SetContentLength(buffer_copy.size());

      auto partStream = Aws::MakeShared<Aws::StringStream>("UploadPartStream");
      partStream->write(reinterpret_cast<const char*>(buffer_copy.data()), buffer_copy.size());
      uploadPartRequest.SetBody(partStream);

      auto outcome = s3client->UploadPart(uploadPartRequest);

      if (outcome.IsSuccess()) {
        Aws::S3::Model::CompletedPart completedPart;
        completedPart.SetPartNumber(current_part);
        completedPart.SetETag(outcome.GetResult().GetETag());

        {
          std::lock_guard<std::mutex> parts_lock(completed_parts_mutex);
          completedParts.push_back(completedPart);
        }

        final_status = IOStatus::OK();
        break;
      } else {
        final_status = IOStatus::IOError(outcome.GetError().GetMessage());
      }
    }

    if (!final_status.ok()) {
      std::cerr << "Error: 分段 " << current_part << " 上传失败，重试 " << kMaxRetries << " 次仍未成功" << std::endl;
    }
    tracker_->TaskFinished(final_status);
  }, ROCKSDB_NAMESPACE::AsyncIOScheduler::QueueType::S3);
}

void BlockBasedTableBuilder::completeupload(){
  tracker_->FinishAddingTasks();
  tracker_->Wait();
  auto s3client = AsyncIOScheduler::Instance().GetS3Client();
  if (completedParts.empty()) {
      std::cerr << "Error: 无法完成上传：无有效分段" << std::endl;
      return;
  }
  ...
  completeRequest.WithBucket(bucket_name)
                  .WithKey(object_key)
  ...
  if (!completeOutcome.IsSuccess()) {
          std::cerr << "Error: 完成上传失败: " << completeOutcome.GetError().GetMessage() << std::endl;
      return;
  }
...
void BlockBasedTableBuilder::WriteMaybeCompressedBlock(
  ...
  {
    if(is_s3 && upload_while_generate){
      partBuffer.insert(partBuffer.end(),
      reinterpret_cast<const unsigned char*>(block_contents.data()),
      reinterpret_cast<const unsigned char*>(block_contents.data()) + block_contents.size());
      if(partBuffer.size()>kUploadThreshold){
        partUpload();
      }
    }else{
      ...
      trailer.data());
  {
    if(is_s3 && upload_while_generate){
      partBuffer.insert(partBuffer.end(),
      reinterpret_cast<const unsigned char*>(Slice(trailer.data(), trailer.size()).data()),
      reinterpret_cast<const unsigned char*>(Slice(trailer.data(), trailer.size()).data()) + Slice(trailer.data(), trailer.size()).size());
      if(partBuffer.size()>kUploadThreshold){
        partUpload();
      }
    }else{
  ...
  if (r->table_options.block_align && is_data_block) {
    ...
    if(is_s3 && upload_while_generate){
      partBuffer.insert(partBuffer.end(), pad_bytes, 0x00);
      if(partBuffer.size()>kUploadThreshold){
        partUpload();
      }
...
void BlockBasedTableBuilder::WriteFooter(BlockHandle& metaindex_block_handle,
  ...
  if(is_s3 && upload_while_generate){
    partBuffer.insert(partBuffer.end(),
    reinterpret_cast<const unsigned char*>(footer.GetSlice().data()),
    reinterpret_cast<const unsigned char*>(footer.GetSlice().data()) + footer.GetSlice().size());
    partUpload();
    completeupload();
    r->set_offset(r->get_offset() + footer.GetSlice().size());
  }else{
    ios = r->file->Append(io_options, footer.GetSlice());
    r->file->GetTracker()->footer_=true;
    if (ios.ok()) {
      r->set_offset(r->get_offset() + footer.GetSlice().size());
    } else {
      r->SetIOStatus(ios);
