...

    {
      ...
      if (perform_data_verification_) {
        ...
        AsyncIOScheduler::Instance().Submit([this, data_copy = std::move(data_copy), offset_copy, opts]() {
          ...
          //               << " (" << data_copy.size() << " bytes): "
          //               << appends.ToString() << std::endl;
          // } else {
          //     std::cout << "write " << file_name_ << " at " << offset_copy
          //               << " with " << data_copy.size() << " Data" << std::endl;
          // }
        },ROCKSDB_NAMESPACE::AsyncIOScheduler::QueueType::EBS);
        // s = writable_file_->PositionedAppend(Slice(src, size), write_offset,
        //                                 opts, nullptr);
        // std::cout << "write " << file_name_ << " at " << write_offset << " with " <<  size <<" Data "<<std::endl;
        s = IOStatus::OK();
        if(size<1024*1024 || tracker_->footer_){
          //std::cout << file_name_ << " Waitting for all tasks down" << std::endl;
          tracker_->FinishAddingTasks();
          tracker_->Wait();
          //std::cout << file_name_ << " 完成" << std::endl;
        }
      }
