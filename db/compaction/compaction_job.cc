...
Status CompactionJob::OpenCompactionOutputFile(SubcompactionState* sub_compact,
  ...
  int path_id = 0;
  std::string fname;
  if(db_options_.cloud_move){
    std::string path;
    if(sub_compact->compaction->output_level() > db_options_.hyper_level){
      path_id = 1;
    }
    bool ebs_free = AsyncIOScheduler::Instance().GetMethod();
    if (ebs_free && sub_compact->compaction->output_level() == (db_options_.hyper_level+1)){
      path_id=0;
    }
    path = sub_compact->compaction->immutable_options()->db_paths[path_id].path;
    fname = MakeTableFileName(path,file_number);
  }else{
    fname = GetTableFileName(file_number);
  }
  // Fire events.
  ColumnFamilyData* cfd = sub_compact->compaction->column_family_data();
