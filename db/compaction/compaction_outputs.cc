...
Status CompactionOutputs::AddToOutput(
    ...
    if (!HasBuilder()) {
      ...
      if(compaction_->immutable_options()->cloud_move && compaction_->immutable_options()->upload_while_generate){
        //std::cout << "is cloud and can upload while generate" <<std::endl;
        FileMetaData& f = current_output().meta;
        int path_id = f.fd.GetPathId();
        if(path_id==1){
          std::string epoch = compaction_->immutable_options()->file_epoch;
          std::string path = compaction_->immutable_options()->db_paths[path_id].path;
          auto pos = path.rfind('/');
          std::string basepath;
          ...
          std::string bucket_name = "testbucket1-jx";
          std::string object_key = MakeTableFileName(basepath,f.fd.GetNumber())+ "-" + epoch;;
          builder_->object_key = object_key;
          builder_->bucket_name = bucket_name;
          builder_->part_number=1;
          builder_->kUploadThreshold = 5 * 1024 * 1024;  // 8MB
          Aws::S3::Model::CreateMultipartUploadRequest createRequest;
          createRequest.WithBucket(bucket_name.c_str())
                        .WithKey(object_key.c_str());
          ...
          builder_->upload_id = uploadid;
          builder_->is_s3 = true;
          builder_->upload_while_generate = compaction_->immutable_options()->upload_while_generate;
        }
      }
    }
