...
IOStatus S3StorageProvider::DoPutCloudObject(const std::string& local_file,
  ...
  if (s3client_->HasTransferManager()) {
    ...
    // bool is_sstable = object_path.find(".sst") != std::string::npos;
    // if(is_sstable){
    ...
    //     std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
    //     Log(InfoLogLevel::ERROR_LEVEL, cfs_->GetLogger(),
    //         "[s3] CreateMultipartUpload %s/%s ERROR %s",
    //         bucket_name.c_str(), object_path.c_str(), errmsg.c_str());
    //     return IOStatus::IOError(local_file, errmsg);
    //   }
    //   std::string upload_id = createOutcome.GetResult().GetUploadId();
    ...
    //   auto parts_uploaded = std::make_shared<std::atomic<int>>(0);
    //   std::vector<Aws::S3::Model::CompletedPart> completed_parts(part_count);

    //   for (int part_number = 1; part_number <= part_count; ++part_number) {
    //     uint64_t offset = (part_number - 1) * part_size;
    //     uint64_t current_part_size = std::min(part_size, file_size - offset);

    //     AsyncIOScheduler::Instance().Submit([=, &completed_parts]() {
    //       std::ifstream file_stream(local_file, std::ios::binary);
    //       if (!file_stream) {
    //         Log(InfoLogLevel::ERROR_LEVEL, cfs_->GetLogger(),
    //             "[s3] Multipart upload failed to open file part %d", part_number);
    //         return;
    //       }
    //       file_stream.seekg(offset);
    //       std::vector<char> buffer(current_part_size);
    //       file_stream.read(buffer.data(), current_part_size);

    //       Aws::S3::Model::UploadPartRequest upload_part_request;
    //       upload_part_request.SetBucket(ToAwsString(bucket_name));
    //       upload_part_request.SetKey(ToAwsString(object_path));
    ...
    //       stream->write(buffer.data(), current_part_size);
    //       upload_part_request.SetBody(stream);
    //       upload_part_request.SetContentLength(current_part_size);

    //       auto upload_part_outcome = s3_client->UploadPart(upload_part_request);
    //       if (!upload_part_outcome.IsSuccess()) {
    //         Log(InfoLogLevel::ERROR_LEVEL, cfs_->GetLogger(),
    //             "[s3] UploadPart failed part %d: %s", part_number,
    ...
    //         completed_parts[part_number - 1] = std::move(completed_part);
    //       }

    //       if (++(*parts_uploaded) == part_count) {
    ...
    //   {
    //     std::unique_lock<std::mutex> lock(*cv_mutex);
    //     cv->wait(lock, [parts_uploaded, part_count]() { 
    //       return *parts_uploaded == part_count; 
    //     });
    //   }
    ...
    //   complete_request.SetBucket(ToAwsString(bucket_name));
    //   complete_request.SetKey(ToAwsString(object_path));
    ...
    //         complete_outcome.GetError().GetMessage().c_str());
    //     return IOStatus::IOError(local_file, complete_outcome.GetError().GetMessage().c_str());
    //   }
    // }else{
      auto inputData =
